...
$.extend($.fn, {
	...
	validate: function( options ) {
		...
		var _form = $(this[0]);
		var _allVerifyELes = _form.find('input,textarea,select').filter(function(){return $(this).attr('_verify');});
		
		_allVerifyELes.each(function(){
			var _ele = $(this);
...
$.extend($.validator, {
	...
	defaults: {
		...
		onfocusin: function(element) {
			...
			if ( this.settings.focusCleanup && !this.blockFocusCleanup ) {
				...
				$(element).parent().removeClass(this.settings.errorClass);
			}
			//$(element).valid();
		},
	...
	messages: {
		...
		_number:"Please enter a valid number or nothing",
		numberDE: "Bitte geben Sie eine Nummer ein.",
		positiveNumber:"Please enter only positive number",
		password:"Please enter more than 8 chars ,only (a-z | 0-9)",
	...
	prototype: {
		...
		checkForm: function() {
			...
			for ( var i = 0, elements = (this.currentElements = this.elements()); elements[i]; i++ ) {
				var result = this.check( elements[i] );
				/////////////////jim
				var _pEle = $(elements[i]).parent();
				if(!result){
		...
		showErrors: function(errors) {
			...
			var _errorList = this.errorList;
			for(var i=0; i < _errorList.length; i++){
				var _pEle = $(_errorList[i].element).parent();
				_pEle.addClass('error');
			}
		...
		showLabel: function(element, message) {
			...
			if ( label.length ) {
				...
				label.attr("generated") && label.html((message&&message.indexOf('<span>') != -1 ? '' : '<span></span>') + message);
			} else {
				// create label
				label = $("<" + this.settings.errorElement + "/>")
					.attr({"for":  this.idOrName(element), generated: true})
					.addClass(this.settings.errorClass)
					.html((message&&message.indexOf('<span>') != -1 ? '' : '<span></span>') + (message || ""));
				if ( this.settings.wrapper ) {
	...
	methods: {
		...
		phone: function(value, element){
			return this.optional(element) || /^(\d{3}-\d{8}|\d{4}-\d{7}|\d{5,12})$/i.test(value);
		},
		fax: function(value, element){
			return this.optional(element) || /^(\d{3}-\d{8}|\d{4}-\d{7}|\d{5,12})$/i.test(value);
		},
		zipcode: function(value, element){
			return this.optional(element) || /^[1-9]\d{5}(?!\d)$/i.test(value);
		},
		...
		_number: function(value, element) {
			return this.optional(element) || (value=='' || /^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/.test(value));
		},
		...
		password:function(value,element){
			...
			/*return this.optional(element) || /^([0-9a-z]{8,})$/.test(value);*/
		},
